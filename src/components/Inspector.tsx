...
import type { Asset, Collection, Tag } from "../types";
...
export function Inspector({ selectionMode, focusedAsset, checkedAssets, tags, collections, ...actions }: Props) {
    ...
    <aside className="inspector">
      <AnimatePresence mode="wait" initial={false}><motion.div key={contentKey} className="inspectorContent" initial={{ opacity: 0, x: 4 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -4 }} transition={{ duration: 0.16, ease: [0.22, 1, 0.36, 1] }}>
        ...
        {!batchMode && <>
          ...
          <div className="assetIdentity"><strong>{asset.filename}</strong><span>{asset.path}</span></div>
          <section className="inspectorSection metadataGrid">
            <h3>{t("details")}</h3>
            <dl><div><dt>{t("type")}</dt><dd>{asset.extension.toUpperCase()}</dd></div><div><dt>{t("size")}</dt><dd>{formatBytes(asset.byteSize)}</dd></div>
              ...
              <div className="wide"><dt>{t("modified")}</dt><dd>{formatDate(asset.modifiedAt)}</dd></div></dl>
          </section>
        </>}
        ...
        {collections.length > 0 && <section className="inspectorSection"><h3>{t("collections")}</h3><div className="collectionPicker">{collections.map((collection) => <button key={collection.id} onClick={() => actions.onAddCollection(collection.id)}><FolderInput size={14} />{collection.name}</button>)}</div></section>}
        {!batchMode && <section className="inspectorSection noteSection"><div className="sectionHeading"><h3>{t("note")}</h3><button className="iconButton tiny" title={t("save")} disabled={note === asset.note} onClick={() => actions.onSaveNote(asset.id, note)}><Save size={14} /></button></div><textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} /></section>}
