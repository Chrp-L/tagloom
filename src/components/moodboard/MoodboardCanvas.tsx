...
import { AlignCenterHorizontal, AlignCenterVertical, BetweenHorizontalStart, BetweenVerticalStart, BringToFront, Download, Expand, Grid3X3, Link2, Magnet, Map as MapIcon, MoreHorizontal, MousePointer2, Palette, Redo2, SendToBack, SlidersHorizontal, Trash2, Type, Undo2 } from "lucide-react";
...
export function MoodboardCanvas({ document, collectionAssets = [], contextSections, searchResults = [], search = "", onSearchChange, allAssets, saveState, onChange, onPreviewAsset, onUndo, onRedo, canUndo, canRedo, onExport, onBoardMenu }: MoodboardCanvasProps) {
  ...
  const commitText = useCallback((id: string, text: string) => {
    ...
    documentRef.current = { ...base, nodes: base.nodes.map((node) => node.id === id && node.type === "text" ? { ...node, data: { ...node.data, text } } : node) };
  ...
  const addSwatch = useCallback(() => { const base = documentRef.current; publish({ ...base, nodes: [...base.nodes, { id: createMoodboardId("swatch"), type: "swatch", position: { x: 152, y: 152 }, size: { width: 160, height: 112 }, zIndex: Math.max(0, ...base.nodes.map((item) => item.zIndex)) + 1, data: { color: "#ed6758", name: "Coral" } }] }); }, [publish]);
