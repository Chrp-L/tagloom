...
pub fn attach(app: AppHandle, state: &AppState, source_id: String, path: &Path) -> AppResult<()> {
    ...
    let mut watcher = notify::recommended_watcher(move |result: notify::Result<notify::Event>| {
        ...
        let Ok(mut last) = emitted_at.lock() else { return; };
        if last.elapsed() < Duration::from_millis(900) { return; }
        *last = Instant::now();
        let _ = app.emit("source-dirty", &event_source);
    }).map_err(|error| AppError::Message(error.to_string()))?;
    watcher.watch(path, RecursiveMode::Recursive).map_err(|error| AppError::Message(error.to_string()))?;
    state.watchers.lock().map_err(|_| AppError::Message("Watcher state is unavailable".into()))?.insert(source_id, watcher);
    Ok(())
}

pub fn detach(state: &AppState, source_id: &str) {
    if let Ok(mut watchers) = state.watchers.lock() { watchers.remove(source_id); }
}
